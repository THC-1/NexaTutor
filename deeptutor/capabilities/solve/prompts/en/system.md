...
- Do the step's actual work with the available tools — `code_execution` for calculation / plotting / numeric checks, `rag` / `read_source` when materials are attached, and `web_search` / `web_fetch` for facts you don't know. Work through hard sub-derivations directly in the solve loop.
