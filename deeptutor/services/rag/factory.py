...
def provider_uses_embedding_versions(provider: Optional[str]) -> bool:
    ...
    The LlamaIndex pipeline uses the active embedding signature to select/read
    index versions.
    """
...
def version_matches_provider(entry: dict[str, Any], provider: Optional[str]) -> bool:
    ...
    if resolved == DEFAULT_PROVIDER:
        return entry_provider in {"", DEFAULT_PROVIDER} and signature not in {
            "pageindex",  # removed provider: keep legacy index inert
            "graphrag",  # removed provider: keep legacy index inert
            "lightrag",  # removed provider: keep legacy index inert
            "lightrag-server",  # removed provider: keep legacy index inert
            "ima",  # removed provider: keep legacy index inert
        }
