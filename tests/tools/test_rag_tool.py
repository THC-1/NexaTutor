...
class TestPipelineFactory:
    def test_list_pipelines_is_llamaindex_only(self) -> None:
    ...
    def test_get_pipeline_normalizes_removed_provider(self) -> None:
        """A removed provider name can only resolve to the standard pipeline."""
        try:
            pipe = get_pipeline("lightrag", kb_base_dir="/tmp/kb-test")
        except (ValueError, ImportError) as exc:
            pytest.skip(f"LlamaIndex dependency/config unavailable: {exc}")
        assert type(pipe).__name__ == "LlamaIndexPipeline"


class TestRAGServiceClassHelpers:
    def test_list_providers_is_llamaindex_only(self) -> None:
