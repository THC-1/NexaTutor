...
async def test_code_execution_tool_runs_python_via_sandbox(
    ...
    assert result.metadata["argv"] == ["python3", "-I", "main.py"]
...
@pytest.mark.asyncio
async def test_code_execution_tool_rejects_cpp() -> None:
    with pytest.raises(ValueError, match="supported: python"):
        await CodeExecutionTool().execute(language="cpp", code="int main(){}")
