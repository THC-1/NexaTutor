...
// bot name, session_id, "session:question" composites, kb_name).
const ENTITY_REF_RE =
  /\b(chat|notebook|quiz|kb|cowriter):[A-Za-z0-9_.\-:]+/g;
        ...
        <div className="relative flex items-start gap-2 rounded-xl border border-[var(--border)] bg-[var(--muted)] px-4 py-3 pr-10 text-[13px]">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[var(--muted-foreground)]">
              {t(
                "Stored at memory/backup/{{name}}. v2 starts fresh — interact with NexaTutor and click Update on each doc to build memory.",
                { name: latestBackup },
              )}
            </p>
          </div>
