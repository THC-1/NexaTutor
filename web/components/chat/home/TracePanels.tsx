...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "NexaTutor Reflecting…"
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "NexaTutor Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "NexaTutor";
  ...
  else if (mode === "responded") modeLabel = t("NexaTutor responded.");
 ...
 * reasoning trace once NexaTutor stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("NexaTutor Exploring… · 8s", settling to "NexaTutor responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while NexaTutor is still reasoning /
